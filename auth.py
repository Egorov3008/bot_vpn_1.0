...
async def login_with_credentials(server_id: str, username: str, password: str):
    """
    Выполняет вход в систему с использованием учетных данных пользователя.

    Args:
        server_id (str): Идентификатор сервера, к которому нужно подключиться.
        username (str): Имя пользователя для аутентификации.
        password (str): Пароль для аутентификации.

    Returns:
        aiohttp.ClientSession: Сессия клиента, используемая для последующих запросов.

    Raises:
        Exception: Если авторизация не удалась, будет выброшено исключение с кодом статуса и текстом ответа.
    """
    global session
...
async def get_clients(session, server_id):
    ...
    Args:
        session (aiohttp.ClientSession): Сессия клиента для выполнения запроса.
        server_id (str): Идентификатор сервера, с которого нужно получить клиентов.

    Returns:
        dict: Словарь с данными о клиентах.

    Raises:
        Exception: Если запрос не удался, будет выброшено исключение с кодом статуса и текстом ответа.
    """
    api_url = SERVERS[server_id]['API_URL']
...
async def link(session, server_id: str, client_id: str, email: str):
    ...
    Raises:
        Exception: Если не удалось получить данные клиентов или если данные клиентов отсутствуют.
    """
...
async def link_subscription(email, server_id):
    """
    Получает URL подписки для указанного клиента.
    ...
    Returns:
        str: URL подписки для клиента.

    Raises:
        ValueError: Если сервер не найден в конфигурации.
    """
